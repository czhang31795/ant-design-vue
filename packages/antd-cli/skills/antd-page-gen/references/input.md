...
> 自动生成自组件库文档，细节请用 `antd-cli components get input` / `antd-cli demos get input/<id>`。
...
- `addon` — 前置/后置标签
- `allow-clear` — 带移除图标
- `autosize-textarea` — 适应文本高度的文本域
- `basic` — 基本用法
- `borderless` — 无边框
- `group` — 输入框组合
- `password-input` — 密码框
- `presuffix` — 前缀和后缀
- `search-input-loading` — 搜索框 loading
- `search-input` — 搜索框
- `show-count` — 带字数提示
- `size` — 三种大小

```bash
antd-cli components get input --json
antd-cli demos list input --json
antd-cli demos get input/<demo-id> --json
```
