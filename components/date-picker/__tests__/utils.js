...
export function hasSelected(wrapper, date) {
  return document.body
    ...
    .getAttribute('class')
    .split(' ')
    .includes('xy-calendar-selected-day');
}

export function openPanel(wrapper) {
  wrapper.find('.xy-calendar-picker-input').trigger('click');
}

export function clearInput(wrapper) {
  wrapper.find('.xy-calendar-picker-clear').trigger('click');
}

export function nextYear() {
  $$('.xy-calendar-next-year-btn')[0].click();
}

export function nextMonth() {
  $$('.xy-calendar-next-month-btn')[0].click();
}

export function selectDateFromBody(date, index) {
  let calendar = document.body;
  if (index !== undefined) {
    calendar = document.body.querySelectorAll('.xy-calendar-range-part')[index];
  }
