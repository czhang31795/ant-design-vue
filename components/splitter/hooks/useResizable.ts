import type { ItemType, ResizableInfo, ShowCollapsibleIconMode } from '../interface';
...
export default function getResizableInfos(
  items: ItemType[],
  pxSizes: number[],
  reverse: boolean,
): ResizableInfo[] {
  ...
  for (let i = 0; i < items.length - 1; i += 1) {
    ...
    const prevSize = pxSizes[i] || 0;
    const nextSize = pxSizes[i + 1] || 0;
