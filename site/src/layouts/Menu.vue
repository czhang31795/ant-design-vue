<template>
  <xy-config-provider :theme="{ components: { Menu: { colorItemBg: colorBgContainer } } }">
    <xy-menu
      :inline-indent="30"
    ...
    >
      <xy-menu-item v-if="showOverview" key="/components/overview">
      ...
      <template v-for="m in menus">
        <template v-if="m.children">
          <xy-menu-item-group :key="m.order" :title="isZhCN ? m.title : m.enTitle">
            <template v-for="n in m.children">
              <xy-menu-item v-if="n.path" :key="n.path">
                ...
                <xy-tag v-if="n.tag" color="green" style="margin-left: auto">{{ n.tag }}</xy-tag>
              </xy-menu-item>
            </template>
          </xy-menu-item-group>
        </template>
        <template v-else>
          <xy-menu-item :key="m.path">
